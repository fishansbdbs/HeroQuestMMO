...
export function applyProgressionStats(player) {
  ...
  const itemMaxMana = Math.max(0, finiteNumber(player.itemMaxMana, 0));
  ...
  const maxMana = BASE_MANA + bonuses.maxMana + itemMaxMana;
  ...
  return {
    ...
    passiveEffects: player.itemPassiveEffects || player.passiveEffects || [],
    movementSpeed: player.movementSpeed || 0,
    cooldownReduction: player.cooldownReduction || 0,
    coinGain: player.coinGain || 0,
    potionPower: player.potionPower || 0,
    questXpGain: player.questXpGain || 0,
    maxMana,
    mana,
    maxHealth,
    health,
